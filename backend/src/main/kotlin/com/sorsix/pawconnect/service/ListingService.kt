...
class ListingService(
    ...
    private fun findListingWithAssociations(id: Long): Listing? = listingRepository.findByIdWithAllAssociations(id)
    ...
    fun createListing(
        ...
        val municipality =
            municipalityRepository.findByCode(request.municipalityCode)
        ...
        val pet =
            when {
                request.petId != null ->
                    petRepository.findByIdOrNull(request.petId)
                        ?: return CreateListingResult.NotFound("Pet not found: ${request.petId}")

                request.pet != null ->
                    when (val result = petService.createPet(request.pet, currentUser)) {
        ...
        if (listingRepository.existsByPet_IdAndStatus_CodeInAndDeletedAtIsNull(
                pet.requireId(),
                ListingStatusCodes.OPEN_STATUSES,
            )
        ) {
    ...
    fun listMyListings(
        currentUser: User,
        pageable: Pageable,
    ): Page<Listing> = listingRepository.findMyListingsWithAssociations(currentUser.requireId(), pageable)

    @Transactional(readOnly = true)
    fun adminSearchListings(
        statusCode: String?,
        pageable: Pageable,
    ): Page<Listing> = listingRepository.findAllWithAssociations(statusCode, pageable)
    ...
    fun cancelOpenListingsForUser(user: User): Int {
        val open =
            listingRepository.findByPostedBy_IdAndStatus_CodeInAndDeletedAtIsNull(
                user.requireId(),
                ListingStatusCodes.OPEN_STATUSES,
            )
    ...
    private fun ownershipDenialReason(
        listing: Listing,
        currentUser: User,
    ): String? = denialReason(currentUser.isAdmin() || listing.postedBy.id == currentUser.id, "You do not own this listing")
}
